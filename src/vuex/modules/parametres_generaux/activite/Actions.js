...
//***************************** importation des ACTIVITES lega**** */

export function importationActivite({ commit }, nouveauObjet) {

    asyncLoading(axios.post('/import_activites', nouveauObjet)).then(res => {
        if (res.status === 201) {
            this.$app.$notify({
                title: 'success',
            ...
            commit('IMPORTATION_PLAN_ACTIVITE', res.data)
        } else {
            this.$app.$notify({
                title: 'Erreur',

...
<template>
   <div>
<div id="DossierMandatAttente" class="modal hide tailgrand">
...
ListePieceJustificative() {
   ...
   listeDossierMandat() {
      return id => {
        if (id != null && id != "") {
           return this.gettersDossierMandat.filter(qtreel => qtreel.demande_engagement_id == id && this.typeProcedure(qtreel.demande_engagement_id) == "Engagement direct" && qtreel.decision_cf==0);
    ...
    SommeDossierMandat() {
      return id => {
        if (id != null && id != "") {
           return this.gettersDossierMandat.filter(qtreel => this.ua(qtreel.ua_id) == id && this.typeProcedure(qtreel.demande_engagement_id) == "Engagement direct" && qtreel.decision_cf==0).reduce((prec,cur) => parseFloat(prec) + parseFloat(cur.total_general), 0).toFixed(0);
...
this.$("#DossierMandatAttente").modal('hide');
    ...
    afficheDecisionCf(id) {
      this.$("#DossierMandatAttente").modal({
        backdrop: "static",
        keyboard: false
      });

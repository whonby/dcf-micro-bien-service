...
<template>
   <div>
<div id="demandeRejetter" class="modal hide tailgrand">
...
ListePieceJustificative() {
   ...
   listeDesDmdParBonCommande() {
      return id => {
        if (id != null && id != "") {
           return this.gettersDemandeEngagement.filter(qtreel => qtreel.ua_id == id && qtreel.type_procedure_id == "Engagement par Bien de Commande" && qtreel.decision_cf==3);
    ...
    SommeDesDmdParBonCommande() {
      return id => {
        if (id != null && id != "") {
           return this.gettersDemandeEngagement.filter(qtreel => qtreel.ua_id == id && qtreel.type_procedure_id == "Engagement par Bien de Commande" && qtreel.decision_cf==3).reduce((prec,cur) => parseFloat(prec) + parseFloat(cur.	total_general), 0).toFixed(0);
...
this.$("#demandeRejetter").modal('hide');

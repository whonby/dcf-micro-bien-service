...
<template>
   <div>
<div id="DossierMandatViseObSER" class="modal hide tailgrand">
                  ...
                  <td>
                            ...
                            <div class="controls">
                              <input type="date" class="span4"  />
                               <input type="hidden" class="span4"  v-model="editDossierMandat.date_motif" />
...
ListePieceJustificative() {
   ...
   listeDossierMandat() {
      return id => {
        if (id != null && id != "") {
           return this.gettersDossierMandat.filter(qtreel => this.ua(qtreel.demande_engagement_id) == id && this.typeProcedure(qtreel.demande_engagement_id) == "Engagement par Bien de Commande" && qtreel.decision_cf==9);
...
this.$("#DossierMandatViseObSER").modal('hide');
    ...
    afficheDecisionCf(id) {
      this.$("#DossierMandatViseObSER").modal({
        backdrop: "static",
        keyboard: false
      });

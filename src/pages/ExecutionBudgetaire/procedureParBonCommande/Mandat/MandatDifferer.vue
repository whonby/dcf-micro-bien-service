...
<template>
   <div>
<div id="DossierMandatDifferer" class="modal hide tailgrand">
...
ListePieceJustificative() {
   ...
   listeDossierMandat() {
      return id => {
        if (id != null && id != "") {
           return this.gettersDossierMandat.filter(qtreel => this.ua(qtreel.demande_engagement_id) == id && this.typeProcedure(qtreel.demande_engagement_id) == "Engagement par Bien de Commande" && qtreel.decision_cf==2);
...
this.$("#DossierMandatDifferer").modal('hide');
    ...
    afficheDecisionCf(id) {
      this.$("#DossierMandatDifferer").modal({
        backdrop: "static",
        keyboard: false
      });

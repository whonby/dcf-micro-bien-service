          ...
          <div class="widget-box">
            ...
            <div class="widget-content nopadding">
              <table class="table table-bordered table-striped">
                ...
                listeMarcheEnExecutionParEntreprise(idEntreprise)"
                 :key="entrep.id">
                  <td>{{afficheAnneeDuMarche(entrep.marche_id) || 'Non renseigné'}}</td>
                   ...
                   <td style="color:red;font-size:14px;text-align:center">{{formatageSomme(afficheMontantReel(entrep.marche_id)) || 'Non renseigné'}}</td>
                    ...
                    <tr>
                     ...
                     <td style="color:red;font-size:14px;text-align:center">{{formatageSomme(montantToutMarcheExecuteParEntrep(idEntreprise))}}</td>
                   </tr>
                </tbody>
              </table>
    ...
    listeMarcheEnExecutionParEntreprise: function () {
                return id => {
                    if (id != "") {
                      // console.log("Marche leste acte effect finnancier")
                        return this.acteEffetFinanciers.filter(idmarche => idmarche.entreprise_id == id && this.afficheAnneeDuMarche(idmarche.marche_id)== this.anneeAmort && idmarche.entreprise_id != null && this.afficheEtatMarche(idmarche.marche_id)== 2)
                    }
                }
            },
            montantToutMarcheExecuteParEntrep: function () {
                return id => {
                    if (id != "") {
                      // console.log("Marche leste acte effect finnancier")
                        return this.acteEffetFinanciers.filter(idmarche => idmarche.entreprise_id == id && this.afficheAnneeDuMarche(idmarche.marche_id)== this.anneeAmort && idmarche.entreprise_id != null && this.afficheEtatMarche(idmarche.marche_id)== 2).reduce((prec, cur) => parseFloat(prec) + parseFloat(cur.montant_act), 0)
                    }
                }
            },

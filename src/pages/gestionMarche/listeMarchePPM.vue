        ...
        <table class="table table-bordered table-striped">
          ...
          <tr>
            <td>
              <div class="control-group">
                ...
                <div class="controls">
                  ...
                  >
                    <option
                      v-for="plans in afficherParUAEnfonctiondesRole"
                      :key="plans.id"
                      :value="plans.id"
                    >
                      {{ plans.libelle }}
            ...
            <td>
              ...
              /> -->
                  <select
                    v-model="editMarche.economique_id"
                    :readOnly="deveroueconomiq"
                    class="span4"
                  >
                    <option
                      v-for="eco in afficherPlanEconomiqueBienService(
                        editMarche.unite_administrative_id
                      )"
                      :key="eco.economique_id"
                      :value="eco.economique_id"
                    >
                      {{ afficherPlanEconomique(eco.economique_id) }}
                    </option>
                  </select>
                </div>
              </div>
          ...
          <tr>
            ...
            <td colspan="">
              ...
              /> -->
                  <select
                    v-model="editMarche.procedure_passation_id"
                    class="span"
                    :readOnly="deverouPassationModi"
                  >
                    <option
                      v-for="plans in procedurePassations"
                      :key="plans.id"
                      :value="plans.id"
                    >
                      {{ plans.libelle }}
              ...
              <div class="control-group">
                ...
                <div class="controls">
                  <select v-model="formData.nature_prix_id" class="span4">
                    <option
                      v-for="plans in gettesrNaturePrix"
                      :key="plans.id"
                      :value="plans.id"
                    >
                      {{ plans.libelle }}
        ...
        <div class="span12">
          <div class="widget-box">
            <div class="widget-title">
              ...
              <h5>
                Liste des March&eacute;s PPM
                <code>({{ marcheHorSibFiltre.length }})</code>
              </h5>
              <div align="right">
                Recherche:
                <input
            ...
            <div class="widget-content nopadding">
              <table class="table table-bordered table-striped">
                ...
                <tbody>
                  <tr
                    class="odd gradeX"
                    v-for="marche in marcheHorSibFiltre"
                    :key="marche.id"
                  >
                    ...
                    <td
                      @dblclick="ModalModifierMarcheHorsPPM(marche.id)"
                      style="text-align: center"
                    >
                      {{
                        afficherModePassation(marche.procedure_passation_id) ||
                        "Non renseigné"
                      }}
                    </td>
                    <!-- <td @dblclick="ModalModifierMarcheHorsPPM(marche.id)">
                  ...
                  {{marche.afficheEconomique.code || 'Non renseigné'}}- {{marche.afficheEconomique.libelle || 'Non renseigné'}}</td> -->
                    <td @dblclick="ModalModifierMarcheHorsPPM(marche.id)">
                      {{ conversionMajiscule(marche.objet)|| "Non renseigné" }}
                    </td>
                    <td @dblclick="ModalModifierMarcheHorsPPM(marche.id)">
...
export default {
  ...
  computed: {
    ...
    ImputationBudgetModifier() {
      const norme = this.budgetGeneral.find(
        (normeEquipe) =>
          normeEquipe.economique_id == this.editMarche.economique_id &&
          normeEquipe.activite_id == this.editMarche.activite_id &&
          normeEquipe.status == "actu"
      );
        ...
        if (id != null && id != "") {
          return this.budgetEclate.filter(
            (element) =>
              ...
              element.grandenature_id == this.formData.gdenature_id
          );
        }
      };
    },
      ...
      if (this.noDCfNoAdmin) {
        ...
        this.marcheHorSibFiltre1.filter((item) => {
          let val = this.getterUniteAdministrativeByUser.find(
            (row) =>
              row.unite_administrative_id == item.objetUniteAdministrative.id
          );
          if (val != undefined) {
            colect.push(item);
            return item;
        ...
        return colect.filter(
          (items) => items.sib == 1 && items.plan_passation_marche_id != null
        );
      } else {
    ...
    afficheLeNomDesProcedure() {
      if (this.afficherMontantImputation(this.ImputationBudget) == "") {
        ...
        this.afficherMontantImputation(this.ImputationBudget) < 10000000
      ) {
        return "Procédure Simplifiée de demande de Cotation(PSC Sans comité)";
      } else if (
        this.afficherMontantImputation(this.ImputationBudget) < 30000000
      ) {
        return "Procédure Simplifiée de demande de Cotation(PSC Avec comité)";
      } else if (
        this.afficherMontantImputation(this.ImputationBudget) < 60000000
      ) {
        return "Procédure Simplifiée à compétition Limitée(PSL)";
      } else if (
        this.afficherMontantImputation(this.ImputationBudget) < 100000000
      ) {
        return "Procédure Simplifiée à compétition Ouverte(PSO)";
      } else if (
        100000000 < this.afficherMontantImputation(this.ImputationBudget)
      ) {
    ...
    afficherTypeMarche() {
      return (id) => {
        if (id != null && id != "") {
          let objet = this.typeMarches.filter(
            (item) =>
              (item.id == id && item.code_type_marche == 1) ||
              item.code_type_marche == 4
          );
          if (objet) {
    ...
    afficherLaListeDesMarche() {
      return this.printMarcheNonAttribue.filter(
        (element) => element.type_marche.code_type_marche == 4
      );
    },
    ...
    montantMarche() {
      return this.afficherLaListeDesMarche.reduce(
        ...
        if (id != null && id != "") {
          return this.budgetGeneral.filter(
            (element) =>
              element.ua_id == id &&
              element.status == "actu" &&
              element.gdenature_id == this.editMarche.gdenature_id
          );
        }
      };
    },
      ...
      return (id) => {
        if (id != null && id != "") {
          return this.afficheBugdetEnCours.filter(
            (element) =>
              element.ua_id == id &&
              element.gdenature_id == this.formData.gdenature_id
          );
        }
      };
    },
    ...
    nbreMarcheExecuter() {
      return this.getActeEffetFinancierPersonnaliser45.filter(
        (recuper) =>
          recuper.marche.attribue == 2 &&
          this.affichertypeMarcheEx(recuper.marche.type_marche_id) == 4
      ).length;
    },
    // afficheMarchExecuter(){
    ...
    afficheMarcheEnCoursContratualisation() {
      return this.afficherLaListeDesMarche.filter(
        (element) =>
          element.attribue == 1 && element.type_marche.code_type_marche == 4
      );
    },
    ...
    afficheMarcheEnPlanification() {
      return this.afficherLaListeDesMarche.filter(
        (element) =>
          element.attribue == 0 && element.type_marche.code_type_marche == 4
      );
    },
  ...
  methods: {
    ...
    ModalModifierMarcheHorsPPM(index) {
      ...
      this.editMarche = this.marcheHorSibFiltre.find(
        (item) => item.id == index
      );
    },

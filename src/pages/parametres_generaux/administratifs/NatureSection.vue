...
<template>
  <div>
        ...
        <div class="span12">
          <div>
            ...
            <h5
              ...
              "
            >
              Liste des natures de section
            </h5>

            <div align="right" style="cursor: pointer">
              <button
                class="btn btn-success"
                @click.prevent="afficherModalAjouterNatureSection()"
              >
                AJOUTER
              </button>
            </div>
            ...
            <div class="widget-content nopadding">
              ...
              >
                ...
                <tbody>
                  <tr
                    class="odd gradeX"
                    v-for="nature_section in partition(
                      localisationsFiltre,
                      size
                    )[page]"
                    :key="nature_section.id"
                  >
                    <td
                      @dblclick="afficherModalModifierTitre(nature_section.id)"
                    >
                      {{ nature_section.code || "Non renseigné" }}
                    </td>
                    <td
                      @dblclick="afficherModalModifierTitre(nature_section.id)"
                    >
                      {{ nature_section.libelle || "Non renseigné" }}
                    </td>
                    <!-- <td> -->
                    <!-- <div class="btn-group"> -->
                    <td>
                      <button
                        @click.prevent="
                          afficherModalModifierTitre(nature_section.id)
                        "
                        class="btn btn-info"
                      >
                    ...
                    <td>
                      <button
                        @click.prevent="
                          supprimerNatureSection(nature_section.id)
                        "
              ...
              <div v-if="localisationsFiltre.length"></div>
              <div v-else>
                <div align="center">
                  <h6 style="color: red">
                    Aucune nature de section enrégistrée
                  </h6>
                </div>
              </div>
            </div>
          ...
          <div class="pagination alternate">
            <ul>
              ...
              <li
                v-for="(titre, index) in partition(localisationsFiltre, size)
                  .length"
                :key="index"
              ...
              <li
                :class="{
                  disabled:
                    page == partition(localisationsFiltre, size).length - 1,
                }"
              >
    ...
    <div id="modifierModal" class="modal hide">
            ...
            <td>
              <div class="control-group">
                ...
                <div class="controls">
                  <input
                    type="text"
                    v-model="editTitre.code"
                    class="span5"
                    placeholder=""
                  />
                </div>
              </div>
              <!-- <div style="color: red" v-if="verifcode1 == true">
                Ce code existe déjà!
              </div> -->
            </td>
          </tr>
      ...
      <div class="modal-footer">
        <button
          v-show="editTitre.code.length && editTitre.libelle.length"
          @click.prevent="modifiernatureSectionLocal(editTitre)"
          class="btn btn-primary"
        >
          Modifier
        </button>
    ...
    <button
      ...
      @shortkey="afficherModalAjouterNatureSection()"
    >
      Open
    </button>
...
export default {
  ...
  computed: {
    ...
    verifcode() {
      if (this.formData.code == "") {
        ...
        let Objet = this.natures_sections.filter(
  ...
  methods: {
    ...
    ...mapActions("parametreGenerauxAdministratif", [
      ...
      "supprimerNatureSection",
      "modifierNatureSection",
    ]),

        ...
        <div>
                                        ...
                                        <download-excel
                                            ...
                                            style="cursor:pointer;"
                                              :fields = "json_fields"
                                              title="Liste Infrastructure "
                                              name ="Liste Infrastructure"
                                              worksheet = "Infrastructure"
                                            :data="getterInfrastrucure">
        ...
        Recherche: <input type="text" v-model="search">
          ...
          <div class="span4">
           ...
           <div class="widget-content nopadding">
            ...
            <div v-if="getterInfrastrucure.length">
            </div>
            <div v-else>
              <div align="center">
        ...
        <div class="pagination alternate">
              <ul>
                <li :class="{ disabled : page == 0 }"><a @click.prevent="precedent()" href="#">Précedent</a></li>
                   <li  v-for="(titre, index) in partition(getterInfrastrucure,size).length" :key="index" :class="{ active : active_el == index }">
                   <a @click.prevent="getDataPaginate(index)" href="#">{{index + 1}}</a></li>
                <li :class="{ disabled : page == partition(getterInfrastrucure,size).length -1 }"><a @click.prevent="suivant()" href="#">Suivant</a></li>
...
<!----- modifier modal debut  ---->
 ...
 <div id="modifierModal" class="modal hide">
            ...
            <div class="control-group">
              <label class="control-label">Code:</label>
              <div class="controls">
                <input type="text" v-model="editTitre.code" class="span" placeholder="" readonly/>
              </div>
            </div>
            <div class="control-group">
              <label class="control-label">Libellé:</label>
           ...
           <div class="modal-footer"> 
             <button  
             @click.prevent="modifierInfrastructureLocal" class="btn btn-primary"
              >Modifier</button>
              <button data-dismiss="modal" class="btn" href="#">Fermer</button> </div>
            </div>
...
// methode pour maper notre guetter
   ...mapGetters('parametreGenerauxAdministratif', ['getterInfrastrucure']) ,
  ...
  methods: {
   ...
   ...mapActions('parametreGenerauxAdministratif', ['getInfrastructure', 
   'ajouterInfrastructure','supprimerInfrastructure', 'modifierInfrastructure']),   
...
base64_encode(file) {
  ...
  // doc.autoTable({ html: this.getterInfrastrucure })
   var data = this.getterInfrastrucure;
   doc.setFontSize(8)
    doc.text(75,10,"LISTE DES INFRASTRUCTURES")
    // doc.addImage(imgData,'JPEG',15,40,180,160)
  doc.autoTable(this.getColumns(),data)
doc.save('Infastructures.pdf')
return 0
},
getColumns() {
    return [
...
partition:partition,
  ...
  getDataPaginate(index){
    ...
    afficherModalajouterInfrastructure(){
...
modifierInfrastructureLocal(){
  this.modifierInfrastructure(this.editTitre);
  this.$("#modifierModal").modal('hide');
   ...
   // fonction pour vider l'input
    ajouterTitreLocal () {
        var nouveauObjet = {
            ...this.formData,
            code:this.TailleTableau
        }
      this.ajouterInfrastructure(nouveauObjet)
...
afficherModalModifierTitre(id){
 ...
 this.$('#modifierModal').modal({
        ...
        this.editTitre = this.getterInfrastrucure.find(items=>items.id==id);

...
export default {
  ...
  computed: {
    ...
    budgetUnitePasAdmin() {
      ...
      this.budgetGeneral
        ...
        .forEach(function (val) {
          let _objet = vm.filtre_unite_admin.find(
            (item) => item.id == val.ua_id
          );
          if (_objet != undefined) {
            connection.push(val);
          }
    ...
    uaBudget() {
      return (ua_id) => {
        if (ua_id) {
          let uniteAd = this.uniteAdministratives.find(
            (item) => item.id == ua_id
          );
    ...
    activiteBudget() {
      return (activite_id) => {
        if (activite_id) {
          let objet = this.getterTousActivite.find(
            (item) => item.id == activite_id
          );
          if (objet != undefined) {
    ...
    ligneBudget() {
      return (economique_id) => {
        if (economique_id) {
          let objet = this.getterTousPlanBudgetaire.find(
            (item) => item.id == economique_id
          );
          if (objet != undefined) {
            return objet.code + " " + objet.libelle;
          }
    ...
    listeUniteAdminPasSection() {
      if (this.formData.section != "") {
        return this.filtre_unite_admin.filter(
          (item) => item.section_id == this.formData.section
        );
      }
    ...
    listePlanBudgetGetataire() {
      return this.getterTousPlanBudgetaire.filter(
    ...
    listeActiviteUA() {
      return (ua_id) => {
        if (ua_id != "") {
          let collection = [];
          let vm = this;
          let objeBudget = this.budgetUniteAdmin(ua_id).filter(
            (item) => item.gdenature_id == this.grand_nature
          );
          objeBudget.forEach(function (val) {
            let activite = vm.getterTousActivite.find(
    ...
    ligneActivite() {
      return (ua_id, activite_id) => {
        if (ua_id != "" && activite_id != "") {
          ...
          let objet_activite = objeBudget.filter(
            (item) => item.activite_id == activite_id
          );
          objet_activite.forEach(function (val) {
            let ligne = vm.getterTousPlanBudgetaire.find(
              (item) => item.id == val.economique_id
            );
      ...
      return (ua_id, activite_id, economique_id) => {
        if (ua_id != "" && activite_id != "") {
          ...
          let objet_activite = objeBudget.filter((item) => {
            if (
              item.activite_id == activite_id &&
              item.economique_id == economique_id
            ) {
              return item;
            }
          });
    ...
    dotationFinalAE() {
      let montant_ae = this.ligneDotationInitialAE(
        this.ua_id,
    ...
    dotationFinalCP() {
      let montant_cp = this.ligneDotationInitialCP(
        this.ua_id,
        this.formData.activite_id,
        this.formData.economique_id
      );
    ...
    totalVariationAE() {
      let initeVal = 0;
      let montantVariationAELigne = this.ligne_selectionne.reduce(function (
        total,
        currentValue
      ) {
      ...
      let montant =
        parseFloat(montantVariationAELigne) + parseFloat(this.variation_ae);
      return montant;
    },
    totalVariationCP() {
      let initeVal = 0;
      ...
      initeVal);
      let montant =
        parseFloat(montantVariationAELigne) + parseFloat(this.variation_cp);
      return montant;
    },
    totalActiveDotationAE() {
      let montant =
        parseFloat(
          this.activiteDotationInitialAE(this.ua_id, this.formData.activite_id)
        ) + parseFloat(this.totalVariationAE);
      return montant;
    },
    totalActiveDotationCP() {
      let montant =
        parseFloat(
          this.activiteDotationInitialCP(this.ua_id, this.formData.activite_id)
        ) + parseFloat(this.totalVariationCP);
      return montant;
    },
  },
  methods: {
    ...
    ...mapActions("uniteadministrative", ["modificationBudgetCharge"]),
    ...
    ajouterLigne(id) {
      ...
      if (obj_sele == undefined) {
        let obje = {
          ...
          dotationAE: this.ligneDotationInitialAE(
            this.ua_id,
            this.formData.activite_id,
            this.formData.economique_id
          ),
          dotationCP: this.ligneDotationInitialCP(
            this.ua_id,
            this.formData.activite_id,
            this.formData.economique_id
          ),
    ...
    supprimerLigne(id) {
      this.ligne_selectionne = this.ligne_selectionne.filter(
        (item) => item.ligne != id
      );
    },
      ...
      if (this.type_modification == "OUI") {
        if (this.totalVariationAE != 0 && this.totalVariationCP != 0) {
          this.$notify({
            title: "ERROR",
            text:
              "Vous n'avez pas le droit d'effectuer la variation a somme non null ",
            type: "error",
          });
      ...
      if (this.type_modification == "NON") {
        if (this.totalVariationAE == 0 && this.totalVariationCP == 0) {
          this.$notify({
            title: "ERROR",
            text:
              "Vous n'avez pas le droit d'effectuer la variation a somme null ",
            type: "error",
          });

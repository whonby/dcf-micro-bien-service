<template>
  <div>
    <table class="table table-bordered table-striped">
      <td colspan="4">
        <div align="center">
          <h4>FICHE DE SUIVI DES MARCHES/CONTRATS - UNITE ADMINISTRATIVE</h4>
        </div>
      </td>
      <tr>
        <td>
    ...
    <div id="printMe">
      <template
        v-if="
          formData.ua_id == '' || formData.typeua_id == '' || formData.exo == ''
        "
      >
        <h5 style="text-align: center; color: red; font-size: 20px">
          VEUILLEZ SELECTIONNER L'UNITE ADMINISTRATIVE SVP
        </h5>
      </template>
      <template
        v-if="
      ...
      <div
        v-for="item in PROCEDURENORMALE(formData.ua_id)"
        :key="item.id"
      ...
      >
        <div class="widget-title">
          ...
          <div
            class="span12"
            style="text-align: center; color: #000000; font-size: 20px"
          >
            <h5>Marche : {{ afficherObjetMarche(item.marche_id) }}</h5>
          </div>
        </div>
        ...
        <table class="table table-bordered table-striped">
          ...
          <tbody>
            <tr
              class="odd gradeX"
              v-for="type in PROCEDURENORMALEMarche(item.marche_id)"
              :key="type.id"
            >
              ...
              <td style="text-align: center">
                {{
                  afficheLibellePassationMarche(
                    afficheidPassationMarche(type.marche_id)
                  ) || "Non renseigné"
                }}
              </td>
              <td>
                {{ formaterDate(type.date_approbation) || "Non renseigné" }}
              </td>
              <td>
                {{ formaterDate(type.date_odre_service) || "Non renseigné" }}
              </td>
              ...
              <td>
                {{
                  LIBELLEBAILLEUR(idBAILLEUR(type.marche_id)) || "Non renseigné"
                }}
              </td>
              <td>
                {{
                  formatageSomme(parseFloat(type.montant_act)) ||
                  "Non renseigné"
                }}
              </td>
              ...
              <td>
                {{
                  formatageSomme(
                    parseFloat(afficheMontantAvenant(type.marche_id))
                  ) || "Non renseigné"
                }}
              </td>
              ...
              <td>
                {{
                  formatageSomme(parseFloat(type.avance_demarrage_ttc)) ||
                  "Non renseigné"
                }}
              </td>
              ...
              <td>
                {{
                  formatageSomme(
                    parseFloat(PayementExecuter(type.marche_id))
                  ) || "Non renseigné"
                }}
              </td>
              <td>
              ...
              <td>
                <button
                  v-if="EtatMarche(type.marche_id) == 2"
                ...
                <button
                  v-else-if="EtatMarche(type.marche_id) == 3"
                  class="btn btn-info"
                >
                  <span title="MARCHE RESILIE">RE</span>
                </button>
                <button
                  v-else-if="EtatMarche(type.marche_id) == 5"
      ...
      <template
        ...
        "
      >
        <h5 style="text-align: center; color: #000000; font-size: 20px">
          MARCHES/CONTRATS ATTRIBUE ISSUS DE LA PROCEDURE SIMPLIFIEE
        </h5>
      </template>
      <div
        v-for="item in PROCEDURESIMPLIFIEE(formData.ua_id)"
        :key="item.id"
        class="widget-content"
      >
        <div class="widget-title">
        ...
        <table class="table table-bordered table-striped">
          ...
          <tbody>
            <tr
              class="odd gradeX"
              v-for="type in PROCEDURESIMPLIFIEEMarche(item.marche_id)"
              :key="type.id"
            >
              <td>{{ type.numero_marche || "Non renseigné" }}</td>
              <td style="text-align: center">
...
export default {
  ...
  computed: {
    ...
    nombreDecompte() {
      return (id) => {
        if (id != null && id != "") {
          return this.decomptefactures.filter((item) => item.marche_id == id)
            .length;
        }
      };
    },
    ...
    afficheNumeroMarche() {
      return (id) => {
        if (id != null && id != "") {
          let odj = this.acteEffetFinanciers.find(
      ...
      return (id) => {
        if (id != null && id != "") {
          return this.acteEffetFinanciers.filter(
            (item) =>
              (item.marche_id == id &&
                item.ua_id == this.formData.ua_id &&
                this.afficheCodePassationMarche(
                  this.afficheidPassationMarche(item.marche_id)
                ) == 9 &&
                this.afficheExoBudgetaire(item.marche_id) ==
                  this.formData.exo) ||
              (item.marche_id == id &&
                item.ua_id == this.formData.ua_id &&
                this.afficheCodePassationMarche(
                ...
                this.afficheExoBudgetaire(item.marche_id) == this.formData.exo)
          );
        }
      };
    },
    ...
    afficherDateapprobation() {
      return (id) => {
        if (id != null && id != "") {
          let odj = this.getterActeEffetFinanciers.find(
            (item) => item.marche_id == id
          );
          if (odj) {
    ...
    MontantBaseMarche() {
      return (id) => {
        if (id != null && id != "") {
          return this.getterActeEffetFinanciers
      ...
      return (id) => {
        if (id != null && id != "") {
          return this.mandats
            .filter(
              (element) =>
                element.entreprise_id == id &&
                this.afficherIdTypeMarche(element.marche_id) ==
                  this.formData.type_marche_id
            )
            .reduce(
              (prec, cur) => parseFloat(prec) + parseFloat(cur.total_general),
              0

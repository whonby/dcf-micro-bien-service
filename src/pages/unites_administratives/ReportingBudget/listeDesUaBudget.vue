...
<template>
    ...
    <div class="container-fluid">
      ...
      <div class="row-fluid">
        <div class="span12">
          ...
          <router-link :to="{ name: 'ReportingBudget', params: { }}"
                 class="btn btn-info ">
 ...
 <span class="icon">
                ...
                <div class="span6">
                    ...
                    <div align="right">
                Recherche:
                <input type="search" placeholder="Nom Unite Administrative" v-model="search" />
              </div>
                </div>
                <div class="span4">
                    <br>
            ...
            >
              <table class="table table-bordered table-striped" id="Nature_section">
                <thead>
                  ...
                  <th style="font-weight: bold;font-size:14px">Budget Initial</th>
                  <th style="font-weight: bold;font-size:14px">Budget Executé</th>
                  <th style="font-weight: bold;font-size:14px">Budget Actuel</th>
                  <!-- <th style="font-weight: bold;font-size:14px">Taux excution</th> -->
                    <th style="font-weight: bold;font-size:14px">Voir Detail</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    class="odd gradeX"
                    v-for="uniteadministrative in partition(rechercheUa,size)[page]"
                    :key="uniteadministrative.id"
                  >
                   
                   <!-- <template v-if="uniteadministrative.type_ua_id = type_Unite_admins.id"> -->
                  
                    <td style="text-align:center;font-weight: bold;font-size:14px">{{uniteadministrative.code }}</td>
                    <td style="font-weight: bold;font-size:14px">{{uniteadministrative.libelle }}</td>
                  <td style="text-align:center;font-weight: bold;font-size:14px">{{formatageSomme(parseFloat(TotalBudgetInitial(uniteadministrative.id)))}}</td>
                    <td style="text-align:center;font-weight: bold;font-size:14px">{{formatageSomme(parseFloat(budgetConsommerBienService(uniteadministrative.id))+parseFloat(budgetConsommerPersonnel(uniteadministrative.id))+parseFloat(budgetConsommerInvertissement(uniteadministrative.id))+parseFloat(budgetConsommerTransfert(uniteadministrative.id))) }}</td>
                  <td style="text-align:center;font-weight: bold;font-size:14px">{{formatageSomme(parseFloat(TotalBudgetInitial(uniteadministrative.id))-parseFloat(parseFloat(budgetConsommerBienService(uniteadministrative.id))+parseFloat(budgetConsommerPersonnel(uniteadministrative.id))+parseFloat(budgetConsommerInvertissement(uniteadministrative.id))+parseFloat(budgetConsommerTransfert(uniteadministrative.id))))}}</td>
                  ...
                  <!-- <td style="text-align:center;font-weight: bold;font-size:14px">{{((parseFloat((parseFloat(budgetConsommerBienService(uniteadministrative.id))+parseFloat(budgetConsommerPersonnel(uniteadministrative.id))+parseFloat(budgetConsommerInvertissement(uniteadministrative.id))+parseFloat(budgetConsommerTransfert(uniteadministrative.id))))/parseFloat(TotalBudgetInitial(uniteadministrative.id)))/100).toFixed(2)}}%</td>
                   -->
                    <td >
                        <router-link :to="{ name: 'DetailBudgetUa', params: { id: uniteadministrative.id }}"
...
import {partition,formatageSomme} from "../../../../src/Repositories/Repository"
...
TotalBudgetInitial() {
 ...
 budgetConsommerDesModule() {
      const val = parseInt(this.budgetConsommerBienService(this.formData.section)) + parseInt(this.budgetConsommerTransfert(this.formData.section)) + parseInt(this.budgetConsommerInvertissement(this.formData.section))+parseInt(this.budgetConsommerPersonnel(this.formData.section));

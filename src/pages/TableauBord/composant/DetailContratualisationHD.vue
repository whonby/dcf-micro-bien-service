...
<script>
    ...
    export default {
        ...
        name: "DetailContratualisationHD",
        data(){
            return {
                status:"",
                info_region:"",
        ...
        computed:{
            ...
            ...mapGetters("bienService", ['marches',"engagements","getMandatPersonnaliserVise",
                "getterImageMarche","acteEffetFinanciers","getActeEffetFinancierPersonnaliser45",
                "typeMarches","avenants","getterListeMarcheTableauBordFiltre","rapportDocuments",
                "getterInfoTableauBordFiltre","getterCojos","mandate","getMandatPersonnaliserVise", "procedurePassations","typeCandidat",'acteDepense',"appelOffres",
                "getterDossierCandidats","marches","gettersOuverturePersonnaliser","getterAnalyseDossiers",
                "getterProceVerballe","gettersCotationPersonnaliser","getterAnoDMPBailleur","getterDemandeAno"]),
            ...
            getDateOuverturePVJugement(){
                return marche_id=>{
                    let objet=this.rapportDocuments.find(item=>item.marche_id==marche_id)
                    if(objet!=undefined){
            ...
            getDateAoDMP(){
                return marche_id=>{
                    let objet=this.getterAnoDMPBailleur.find(item=>item.marche_id==marche_id)
                    if(objet!=undefined){
                        return objet
                    }
                    return null
            ...
            getPVJugement(){
                return marche_id=>{
                    let objet=this.getterProceVerballe.find(item=>item.marche_id==marche_id);
                    if(objet!=undefined){
                        return objet
                    }
                    return null

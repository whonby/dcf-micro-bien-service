    ...
    export default {
        ...
        computed:{
            ...
            ...mapGetters("bienService", ['marches',"engagements","getMandatPersonnaliserVise",
                ...
                "getterListeMarcheTableauBordFiltre","getterInfoTableauBordFiltre"]),
            ...mapGetters("gestionMarche", ['secteur_activites', 'entreprises']),

            getActEffefinancierMarche(){
                return id=>{
            ...
            tauxAvenantPasMarche(){
                return marche_id=>{
                    let taux=(this.montantTCTAvenant(marche_id)*100)/this.montantApprouveMarche(marche_id)

                    return taux.toFixed(2)
                }
            },

            tauxExecutionPasMarcheFinnancier(){
                return marche_id=>{
                    let taux=(this.montantOPVise(marche_id) * 100)/this.montantApprouveMarche(marche_id)
                    return taux.toFixed(2)
                }
            },
            getEntrepriseAttributaire(){
                return id=>{
                    let objet=this.entreprises.find(item=>item.id==id)
                    if(objet!=undefined){
                        return objet
                    }
                    return null
            ...
            montantApprouveMarche(){
                return marche_id=>{
                    let vm=this;
                    let montantTotal=0;
                    this.getterListeMarcheTableauBordFiltre.filter(item=>item.id==marche_id).forEach(function (val) {
            ...
            tauxFinancementParEtat(){
                return marche_id=>{

                    let taux=(this.montantOPVisePartEtat(marche_id) * 100)/this.montantApprouveMarche(marche_id)
                    return taux.toFixed(2)
                }
            },
            montantOPVisePartEtat(){
                return marche_id=>{
                    let montant_execute=0;
                    let vm=this;
                    let initeVal = 0;
                    let montant=vm.decomptefactures.filter(item=>item.marche_id==marche_id).reduce(function (total, currentValue) {
                        return total + parseFloat(currentValue.parts_etat) ;
                    }, initeVal);
                    montant_execute=parseFloat(montant_execute) + parseFloat(montant)
            ...
            tauxFinancementParBailleur(){
                return marche_id=>{

                    let taux=(this.montantOPVisePartBailleur(marche_id) * 100)/this.montantApprouveMarche(marche_id)
                    return taux.toFixed(2)
                }
            },
            montantOPVisePartBailleur(){
                return marche_id=>{
                    let montant_execute=0;
                    let vm=this;
                    let initeVal = 0;
                    let montant=vm.decomptefactures.filter(item=>item.marche_id==marche_id).reduce(function (total, currentValue) {
                        return total + parseFloat(currentValue.parts_bailleur) ;
                    }, initeVal);
                    montant_execute=parseFloat(montant_execute) + parseFloat(montant)
            ...
            montantRestePaye(){
                return marche_id=>{
                    return this.montantApprouveMarche(marche_id)-this.montantOPVise(marche_id)
                }
            },
            tauxRestePaye(){
                return marche_id=>{
                    let taux=(this.montantRestePaye(marche_id) * 100)/this.montantApprouveMarche(marche_id)
                    return taux.toFixed(2)
                }
            },
            ...
            montantTCTAvenant(){
                return marche_id=>{
                    let vm=this;
                    let montant_avenant=0;
                    let objetAvenant=vm.avenants.filter(item=>item.marche_id==marche_id)
                    if(objetAvenant!=undefined){
                        let initeVal = 0;
                        montant_avenant=objetAvenant.reduce(function (total, currentValue) {
                            return total + parseFloat(currentValue.montant_avenant) ;

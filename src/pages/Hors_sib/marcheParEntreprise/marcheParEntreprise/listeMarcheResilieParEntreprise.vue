                  ...
                  <td>{{afficheAnneeDuMarche(entrep.marche_id) || 'Non renseigné'}}</td>
                   ...
                   <td style="color:red;font-size:14px;text-align:center">{{formatageSomme(parseFloat(afficheMontantReel(entrep.marche_id))) || 'Non renseigné'}}</td>
                    ...
                    <tr>
                     ...
                     <td style="color:red;font-size:14px;text-align:center">{{formatageSomme(parseFloat(montantToutMarcheResilieParEntrep(idEntreprise)))}}</td>
                   </tr>
                </tbody>
              </table>
    ...
    listeMarcheEnResilieParEntreprise: function () {
                return id => {
                    if (id != "") {
                      // console.log("Marche leste acte effect finnancier")
                        return this.acteEffetFinanciers.filter(idmarche => idmarche.entreprise_id == id && this.afficheAnneeDuMarche(idmarche.marche_id)== this.anneeAmort && idmarche.entreprise_id != null && this.afficheEtatMarche(idmarche.marche_id)== 3 && this.afficheMarcheGestionSib(idmarche.marche_id)== 1)
                    }
                }
            },
              montantToutMarcheResilieParEntrep: function () {
                return id => {
                    if (id != "") {
                      // console.log("Marche leste acte effect finnancier")
                        return this.acteEffetFinanciers.filter(idmarche => idmarche.entreprise_id == id && this.afficheAnneeDuMarche(idmarche.marche_id)== this.anneeAmort && idmarche.entreprise_id != null && this.afficheEtatMarche(idmarche.marche_id)== 3 && this.afficheMarcheGestionSib(idmarche.marche_id)== 1).reduce((prec, cur) => parseFloat(prec) + parseFloat(cur.montant_act), 0)
                    }
                }
            },
   anneeAmort() {

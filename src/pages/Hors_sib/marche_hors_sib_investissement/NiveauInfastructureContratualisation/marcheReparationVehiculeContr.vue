...
<template>
              ...
              <h5>Marché Réparation Véhicule</h5>
              <div align="right">
                Recherche:
       ...
       <option value="100">100</option>
           ...
           Entrer
             ...
             <table class="table table-bordered table-striped" >
                ...
                <tbody>
                   <tr class="odd gradeX" v-for="activites in partition (marcheReparationVehiculeToutMarche, size)[page]"
                 :key="activites.id">
...
<td>
   <router-link :to="{ name: 'CycleDeVie', params: { id: activites.id }}"
        ...
        <span class=""><i class=" icon-calendar"></i></span>
           ...
           <li  v-for="(titre, index) in partition(marcheReparationVehiculeToutMarche,size).length" :key="index" :class="{ active : active_el == index }">
           <a @click.prevent="getDataPaginate(index)" href="#">{{index + 1}}</a></li>
            <li :class="{ disabled : page == partition(marcheReparationVehiculeToutMarche,size).length -1 }"><a @click.prevent="suivant()" href="#">Suivant</a></li>
           </ul>
        </div>
                        </div>
...
afficheLocalisation() {
 ...
 marcheReparationVehiculeToutMarche() {
  ...
  return this.marcheReparationVehiculeToutMarche.filter(element => element.attribue == 0 && this.recupererCodeTypeMarche(element.type_marche_id) == 3  && element.sib==1 && element.infrastructure_id==null).reduce((prec, cur) => parseFloat(prec) + parseFloat(cur.montant_marche), 0)
},
  ...
  afficherListeMarcheHorsSib() {
       // const st = this.search.toLowerCase();
        if (this.noDCfNoAdmin){
            ...
            return colect.filter(element => element.attribue == 1 && this.recupererCodeTypeMarche(element.type_marche_id) == 3 && element.parent_id == null && element.sib==1 && element.infrastructure_id == null && element.economique_id == this.CodeExempte(element.economique_id))
        ...
        return this.gettersMarcheHorsib.filter(element => element.attribue == 1 && this.recupererCodeTypeMarche(element.type_marche_id) == 3 && element.parent_id == null && element.sib==1 && element.infrastructure_id==null)
            // return (
            //     items.secti.nom_section.toLowerCase().includes(st) ||
            //     items.libelle.toLowerCase().includes(st)
            // );

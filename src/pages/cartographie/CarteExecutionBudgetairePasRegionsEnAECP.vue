                    ...
                    <div class="sidebar-content">
                        <div class="sidebar-pane" id="home">
                            ...
                            <div class="span5">
                                ...
                                <h6>Grande Nature</h6>
                                <label for="tous">
                                    <input type="radio" v-model="grand_nature_depense" value="" id="tous"> <span>Affiché tous  <b></b></span>
                                </label>
                                <label  v-for="item in grandes_natures" :key="item.id" :for="item.id">
                                    <input type="radio" v-model="grand_nature_depense" :value="item.id" :id="item.id"> <span> {{item.libelle}} <b></b></span>
                                </label>
                            ...
                            <div>
                                <div class="span12" style="font-size: 15px ; font-size: 15px">
                                    Situation du budget</div>
                            </div>

                            <hr>
                            <div id="chart">
                                <apexchart type="pie" width="350" :options="chartOptions3" :series="[montant_budget_restan,montant_budget_execute]"></apexchart>
                            </div>
                        </div>
...
// methode pour maper notre guetter
            ...mapGetters('parametreGenerauxAdministratif', ['structures_geographiques',
                'localisations_geographiques',"getterLocalisationGeoAll","getterInfrastrucure","grandes_natures",
            ...
            //Recuperation du budget pas zone geographique previsionnel en AE CP
               totalBudgetGeneral(){
                   ...
                   let montant_cp=this.budgetGeneral
                       .filter((item) =>item.actived == 1)
                       .reduce(
                           (prec, cur) => parseFloat(prec) + parseFloat(cur.cp),
                           0
                       )
                       .toFixed(0);
            ...
            budgetPrevuesionnelPasRegion(){
              return region_id=>{
                   ...
                   let vm=this;
                   let objetDepartement=this.localisations_geographiques.filter(item=>item.parent==region_id)


                    let montant=0;
                   if(objetDepartement==undefined) return 0;
                  objetDepartement.forEach(function (val) {
                      let objetSous=vm.localisations_geographiques.filter(item=>item.parent==val.id)
                      ...
                      if (objetSous==undefined){
                          ...
                          objetSous.forEach(function (row) {
                              ...
                              montant=parseFloat(montant) + parseFloat(vm.montantPasSousPresefcture(row.id))
                          })
                      }
    ...
    montantPasSousPresefcture(){
      return region_id=>{
          ...
          let montant_ae=this.budgetFiltre
              .filter((item) => item.zone_id == region_id && item.actived == 1)
              .reduce(
                  (prec, cur) => parseFloat(prec) + parseFloat(cur.Dotation_Initiale),
                  0
              )
          ...
          let montant_cp=this.budgetFiltre
              .filter((item) => item.zone_id == region_id && item.actived == 1)
              .reduce(
                  (prec, cur) => parseFloat(prec) + parseFloat(cur.cp),
                  0
          ...
          return parseFloat(montant_cp);
            ...
            budgetFiltre(){
                let vM=this;
                let objet=this.budgetGeneral.filter(item=>item.actived == 1)
                ...
                if(vM.unite_administrative_id!=""  && vM.grand_nature_depense==""){
                    objet =objet.filter(item=>{
                        if(item.ua_id==vM.unite_administrative_id){
                            return item
                        }
                    })
                }
                ...
                if (vM.grand_nature_depense!="" && vM.unite_administrative_id==""){
                    objet =objet.filter(item=>{
                        if(item.gdenature_id==vM.grand_nature_depense){
                            return item
                        }
                    })
                }
                ...
                if(vM.unite_administrative_id!="" && vM.grand_nature_depense!=""){
                    objet =objet.filter(item=>{
                        if(item.ua_id==vM.unite_administrative_id && item.gdenature_id==vM.grand_nature_depense){
                            return item
                        }
                    })
                }
            ...
            objetMarchePasUniteOuRegion(){
                ...
                if(vM.region!="" && vM.unite_administrative_id=="" && vM.grand_nature_depense==""){
                    objet =objet.filter(item=>{
                ...
                //retourne les marches d'une unite administrative selectionner
                if(vM.unite_administrative_id!="" && vM.region=="" && vM.grand_nature_depense==""){
                    objet =objet.filter(item=>{
                ...
                //retourne les marches d'une une infrastucture selectionner
                if (vM.grand_nature_depense!="" && vM.unite_administrative_id=="" && vM.region==""){
                    objet =objet.filter(item=>{
                        if(item.gdenature_id==vM.grand_nature_depense && item.parent_id!=""){
                            return item
                        }
                    })
                }
                ...
                if(vM.unite_administrative_id!="" && vM.region=="" && vM.grand_nature_depense!=""){
                    objet =objet.filter(item=>{
                        if(item.unite_administrative_id==vM.unite_administrative_id && item.gdenature_id==vM.grand_nature_depense && item.parent_id!=""){
                            return item
                        }
                    })
                }
                if(vM.unite_administrative_id=="" && vM.region!="" && vM.grand_nature_depense!=""){
                    objet =objet.filter(item=>{
                        if(item.gdenature_id==vM.grand_nature_depense && item.localisation_geographie_id==vM.region && item.parent_id!=""){
                            return item
                        }
                    })
                }
                ...
                if(vM.unite_administrative_id!="" && vM.region!="" && vM.grand_nature_depense!="" ){
                    objet =objet.filter(item=>{
                        if(item.gdenature_id==vM.grand_nature_depense && item.unite_administrative_id==vM.unite_administrative_id && item.localisation_geographie_id==vM.region && item.parent_id!=""){
                            return item
                        }
                    })
                }
                          ...
                          let  color="#000000"
                           let colorFill="#000000"
                            let objetAlocalise={
                                ...
                                ville:value.libelle,
                                latlng:coordonne,
                                budget:parseFloat(vM.budgetPrevuesionnelPasRegion(value.id)),
                                budgetReste:budgetRest,
                                budgetExecute:vM.montantExecuterPasRegion(value.id),
                                tauxBudget:taux.toFixed(2),
                                color:color,
                                colorFill:colorFill
                            }
                            vM.montant_budget_restan=parseFloat(vM.montant_budget_restan) + parseFloat(budgetRest)
                            vM.montant_budget_execute=parseFloat(vM.montant_budget_execute) + parseFloat(vM.montantExecuterPasRegion(value.id))
                ...
                if(vm.objet_map!="" && vm.objet_leaflet!=""){
                    ...
                    if(this.localisation.length>0){
                        ...
                        this.localisation.forEach(function (value){
                            ...
                            let taux_region_budget=(parseFloat(value.budget) * 100) / parseFloat(vm.totalBudgetGeneral)
                            let taux_region_budget_restant=(parseFloat(montant_restant) * 100) / parseFloat(value.budget)

                            let taux_region_budget_execute=(parseFloat(montant_execute) * 100) / parseFloat(value.budget)

          ...
          <div class="sidebar-content">
            <div class="sidebar-pane" id="home">
              <h1 class="sidebar-header">
                Carte budget previsionnel par regions et pas infrastructure
              ...
              <div class="span5">
                ...
                <label
                  v-for="item in getterInfrastrucure"
                  :key="item.id"
                  :for="item.id"
                >
                  <input
        ...
        if (value.structure_localisation_geographique_id == 5) {
          if (value.longitude != null && value.latitude != null) {
            ...
            let uniteByZoneGeo = vM.uniteAdministratives.filter(
              (item) => item.localisationgeo_id == value.id
            );
      ...
      if (vm.objet_map != "" && vm.objet_leaflet != "") {
        ...
        if (this.localisation.length > 0) {
          this.localisation.forEach(function (value) {
            ...
            if (vm.infrastructure != "") {
              taux =
                (vm.totalMontantInfrastructurePasRegion(value.id) /
                  vm.totalMontantInfrastructure(vm.unite_administrative_id)) *
                100;
            } else {
              taux =
                (vm.totalMontantPrevisionnelPasRegion(value.id) /
                  vm.totalMontantPrevisionnelPasMarche) *
                100;
            }

            vm.getInfrastructure(vm.infrastructure).forEach(function (val) {
              montantInfraParRegion = vm.getMontantMarcheRegionInfrastructure(
                value.id,
                val.id
              );

            ...
            <div class="widget-content nopadding" ref="content">
              <table class="table table-bordered table-striped">
                ...
                <tr class="odd gradeX" v-for="titre in titreFiltres " :key="titre.id">
                  ...
                  <td  @dblclick="afficherModalModifierTitre(titre.id)" v-if="titre.role">{{titre.role.libelle}}</td>
                  <td>
...
<!--                    </router-link>-->
                    <div class="btn-group">
                      <button @click.prevent="supprimerEquipeCF(titre.id)"  class="btn btn-danger ">
...
export default {
  ...
  created() {
    ...
    this.formData.cf_id= this.detail.id
    ...
    titreFiltres(){
      ...
      if(objet.length>0){
        return objet.filter((item) => {
              return item.membre.matricule.toLowerCase().includes(searchTerm)
                  || item.membre.name.toLowerCase().includes(searchTerm)
            }
        )
      }
     return null;
  ...
  methods: {
    ...mapActions('Utilisateurs', ['getUtilisateurs',"getRoles",
      "modifierEquipeCF","supprimerEquipeCF","ajouterEquipeCF"]),
    ...
    ajouterCF(){
      this.ajouterEquipeCF(this.formData)
